...
\pagestyle{fancy}
\lhead{\kaishu 中山大学物理与天文学院基础物理实验\uppercase\expandafter{\romannumeral1}} % 左边页眉，学院 + 课程
\rhead{\kaishu 复摆与混沌} % 右边页眉，实验报告标题
...
% ---------------------------------------------------------------------
% 安全插图：若图片缺失则给出占位框，避免编译失败
% 用法：\safegraphics[<includegraphics 可选参数>]{<文件名及后缀>}
\makeatletter
\newcommand{\safegraphics}[2][]{%
    \IfFileExists{#2}{% 文件存在
        \includegraphics[#1]{#2}%
    }{% 文件不存在——放置占位框
        \begingroup
            \setlength{\fboxsep}{0pt}%
            \fbox{\parbox[c][0.22\textheight][c]{0.8\textwidth}{\centering 缺少图片：\texttt{#2}}}%
        \endgroup
    }%
}
\makeatother